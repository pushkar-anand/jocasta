...
func TestBuildHostRejectsAMalformedMAC(t *testing.T) {
	...
	tests := []struct {
		...
		// but a truncated value must still be refused. Padding it would invent
		// octets.
...
// A randomised address is generated by the device, so it matches no registry
// and never will. That is worth telling apart from a lookup miss, which
// suggests a stale table.
...
// A vendor can use a locally administered prefix without registering it, and
// the table carries those. Such a device is that vendor's hardware,
...
// A supplied name, such as a DHCP lease's host-name, is what a source already
// knows, and outranks reverse DNS. It must survive construction, and it must
// not cost a lookup: the router's tables run to hundreds of rows and every one
...
// The accessors read the parsed fields, which are empty on the zero value.
