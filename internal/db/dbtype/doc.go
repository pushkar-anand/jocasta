...
// does not: it stores a time.Time in Go's String form and has no conversion at
// all from a TEXT column into one.
// The enum columns store their values in upper case, so that query output tells
// a token the schema constrains apart from free text at a glance. Go has no
// convention for the string a constant holds -- MixedCaps governs the
// identifier -- so the identifiers stay MixedCaps over upper-case values, as
// http.MethodGet does for "GET".
package dbtype
