...
// eventPageSize is the page a call that asks for none gets, and
// eventPageLimit the most one may ask for: the same window and ceiling
// GET /api/events uses.
const (
	eventPageSize  = 50
	eventPageLimit = 500
...
func listEvents(store *inventory.Store) func(*mcpsdk.Server, *slog.Logger) {
	...
	handler := func(
		...
		if err := cursor.Decode(in.Cursor); err != nil {
			return nil, listEventsOutput{}, badCursor()
		}

		// Asking for a device that does not exist is a 404, as it is for the
		// API, rather than an empty page that reads as "nothing changed".
		if in.DeviceID != 0 {
		...
		page, err := store.ListEvents(ctx, inventory.Page{
			Limit:          cmp.Or(in.Limit, eventPageSize),
			Cursor:         cursor,
			Device:         in.DeviceID,
			EventKinds:     kinds,
...
// badCursor is the problem a cursor this server did not issue gets. It is a
// Problem already, so addTool passes it to the agent as it stands.
func badCursor() response.Problem {
	return response.NewProblem().
		WithStatus(http.StatusBadRequest).
		WithDetail("cursor is not one a previous list_events call returned").
		Build()
}
...
func listEventsSchema() *jsonschema.Schema {
	...
	s.Properties["limit"].Maximum = new(float64(eventPageLimit))
